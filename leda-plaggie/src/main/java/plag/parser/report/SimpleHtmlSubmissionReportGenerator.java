 ...
 *
 */
public class SimpleHtmlSubmissionReportGenerator implements SubmissionReportGenerator {

	/**
	 * Used in naming the sub directories when generating report of several
	 * submissions
	 */
	...
	/**
	 * The String to prepend to each line of multiline code, if it is printed
	 */
	...
	public SimpleHtmlSubmissionReportGenerator(File baseDir, CodeTokenizer codeTokenizer, int maxDetResultsToReport) {
	...
	/**
	 * Changes the ampersand, less than and greater than characters to their
	 * html form in the givern string.
	 */
	...
	/**
	 * Generates the index.html file for the single submission detection report
	 * page.
	 *
	 * @param file
	 *            is the file to be generated
	 * @param topFile
	 *            is the HTML file appearing in the top fram
	 * @param leftFile
	 *            is the HTML file appearing in the left frame
	 * @param rightFile
	 *            is the HTML file appearing in the right frame
	 * @param detResult
	 *            is the detection result appearing on this page
	 */
	private void generateFramesetIndex(File file, String topFile, String leftFile, String rightFile,
			SubmissionDetectionResult detResult) throws Exception {
		...
		if (detResult.getFileDetectionResults().size() > 0) {
			...
			// If there are no file results, the frameset only contains the top
			// file
			f.println("<frameset>");
			f.println("  <frame SRC=\"" + topFile + "\" name=\"top\">");
			f.println("</frameset>");
	...
	/**
	 * Generates the file appearing in the top frame in the single submission
	 * detection result report.
	 */
	private void generateTopFile(File file, SubmissionDetectionResult detResult) throws Exception {
		...
		f.println("<br>Similarity:<b>" + Stats.getPercentage(detResult.getSimilarityA() * detResult.getSimilarityB())
				+ "</b>");
		...
		f.println("<p>");
		f.println("<A TARGET=\"left\" HREF=\"" + "../sub" + (Integer) subNumbers.get(detResult.getSubmissionA())
				+ "/simdistr.html" + "\">Similarity distribution of " + subAName + "</A><BR>");
		f.println("<A TARGET=\"right\" HREF=\"" + "../sub" + (Integer) subNumbers.get(detResult.getSubmissionB())
				+ "/simdistr.html" + "\">Similarity distribution of " + subBName + "</A><BR>");
	...
	/**
	 * Creates the pages containing the program code of the files with tags in
	 * order to easily locate the similar sections. Creates two files named
	 * filea[counter].html and fileb[counter].html in the given directory.
	 */
	public void generateFiles(File dir, DetectionResult dR, int counter) throws Exception {
		PrintStream fa = new PrintStream(
				new FileOutputStream(dir.getPath() + File.separator + "filea" + counter + ".html"));
		PrintStream fb = new PrintStream(
				new FileOutputStream(dir.getPath() + File.separator + "fileb" + counter + ".html"));
		...
		while ((line = br.readLine()) != null) {
			lineCount++;
			if ((!withinMatch) && (mt != null) && (mt.getTileA().getStartToken().getStartLine() == lineCount)) {
				...
				while ((mt != null) && (mt.getTileA().getEndToken().getEndLine() == lineCount)) {
					...
					if (it.hasNext()) {
						...
						if (mt.getTileA().getStartToken().getStartLine() == lineCount) {
							// And the next match starts on this same line,
							// too...
							withinMatch = true;
							matchCount++;
							fa.print("<A NAME=\"match" + mt.getId() + "\"></A>");
			...
			} else if ((withinMatch) && (mt.getTileA().getEndToken().getEndLine() == lineCount)) {
				...
				// Scan through all the remaining matches, that happen to end on
				// this same line
				while ((mt != null) && (mt.getTileA().getEndToken().getEndLine() == lineCount)) {
					withinMatch = false;
					if (it.hasNext()) {
		...
		// NOW, BE VERY SCARED, THIS GETS EVEN UGLIER -- COPY PASTING THE
		// PREVIOUS CODE WITH SOME
		...
		while ((line = br.readLine()) != null) {
			lineCount++;
			if ((!withinMatch) && (mt != null) && (mt.getTileB().getStartToken().getStartLine() == lineCount)) {
				// We moved into new matched tile
				withinMatch = true;
				matchCount++;
			...
			} else if ((withinMatch) && (mt.getTileB().getEndToken().getEndLine() == lineCount)) {
				// We are on the last line of a match
				withinMatch = false;
				fb.print(mt.getId());
				if (it.hasNext()) {
	...
	/**
	 * Generates a StringBuffer of the lines read from the given buffered
	 * reader.
	 */
	private StringBuffer getChars(BufferedReader br) throws IOException {
	...
	/**
	 * Returns the substring in chars starting from leftChar and ending at
	 * rightChar
	 */
	...
	/**
	 * Prints the tokens and the associated code specified in the given token
	 * list using the file read from the given reader.
	 */
	private void printTokens(TokenList tokens, BufferedReader br, PrintStream out, boolean printLineNumbers,
			int multiLineThreshold) throws IOException {
		...
		while (iter.hasNext()) {
			...
			// if (printLineNumbers) {
			ps += lineLeft + "(" + leftChar + ")-" + lineRight + "(" + rightChar + "):";
			// }
			...
			if (multiLineThreshold > 0) {
				...
				if (lineLeft == lineRight) {
					...
					start = end + 1;
					curCount++;
					while ((curCount < multiLineThreshold) && ((end = s.indexOf("\n", start)) != -1)) {
						out.print(createEmptyString(COL_LENGTH) + PRE_MULTILINE + txt2html(s.substring(start, end))
								+ separator);
						start = end + 1;
						curCount++;
					}
					...
					if (curCount < multiLineThreshold) {
						out.print(createEmptyString(COL_LENGTH) + PRE_MULTILINE + txt2html(s.substring(start))
								+ separator);
					}
				}
			} else {
	...
	/**
	 * Generates the files containing a list of tokens, that have appeared in
	 * the files stored in the given detection result. The generated files are
	 * called tokensa[conter].html and tokensb[counter].html.
	 */
	private void generateTokenLists(File dir, DetectionResult dR, int counter) throws Exception {
		...
		PrintStream prA = new PrintStream(
				new FileOutputStream(dir.getPath() + File.separator + "tokensa" + counter + ".html"));
		PrintStream prB = new PrintStream(
				new FileOutputStream(dir.getPath() + File.separator + "tokensb" + counter + ".html"));

		this.printTokens(tokensA, brA, prA, true, 1);

		this.printTokens(tokensB, brB, prB, true, 1);
	...
	/**
	 * Generates a list of matched tiles, that have appeared in the two files.
	 * The generated file is matchedtiles[counter].html.
	 */
	private void generateMatchedTiles(File dir, DetectionResult dR, int counter) throws Exception {
		PrintStream pr = new PrintStream(
				new FileOutputStream(dir.getPath() + File.separator + "matchedtiles" + counter + ".html"));
		pr.println("<HTML>");
		pr.println("<HEAD>");
		pr.println("</HEAD>");
		pr.println("<BODY>");
		...
		while (i.hasNext()) {
			...
			pr.print("<A HREF=\"tokensa" + counter + ".html#token" + mt.getTileA().getStartTokenIndex()
					+ "\" TARGET=\"left\">");
			pr.print("token left");
			pr.print("</A>");
			...
			pr.print("<A HREF=\"tokensb" + counter + ".html#token" + mt.getTileB().getStartTokenIndex()
					+ "\" TARGET=\"right\">");
	...
	/**
	 * Generates a report of the given single submission detection result.
	 */
	public void generateReport(SubmissionDetectionResult detResult) throws Exception {
		...
		this.generateFramesetIndex(new File(dir.getPath() + File.separator + "index.html"), "top.html", "filea1.html",
				"fileb1.html", detResult);

		// Generate the index frame with list of all matched tiles in all the
		// file detection results
		this.generateTopFile(new File(dir.getPath() + File.separator + "top.html"), detResult);
	...
	/**
	 * Generates the index page of the report, when there are several detection
	 * results to report. This is used to create different index pages for
	 * different sort orders (sort order of detResults changes, as doues the
	 * filename).
	 */
	public void generateIndexPage(SortedSet detResults, HtmlPrintable configuration, HtmlPrintable statistics,
			String filename, HashMap detNumbers) throws Exception {
		...
		f.println("" + "[<A HREF=\"#configuration\">Configuration</A>] " + "[<A HREF=\"#statistics\">Statistics</A>] "
		...
		f.println("<TR>" + "<TD>Student A</TD><TD>Student B</TD>"
				...
				+ "<TD>Student B similarity distribution average</TD>" + "</TR>");
		...
		while (iter.hasNext()) {
			...
			f.print("<A HREF=\"sub" + subNumbers.get(detResult.getSubmissionA()) + "/simdistr.html\">"
					+ Stats.getPercentage(Stats.getDistributionAverage(detResult.getSubmissionA().getName())) + "</A>");

			f.println("</TD><TD>");

			f.print("<A HREF=\"sub" + subNumbers.get(detResult.getSubmissionB()) + "/simdistr.html\">"
					+ Stats.getPercentage(Stats.getDistributionAverage(detResult.getSubmissionB().getName())) + "</A>");
	...
	/**
	 * Orders submission detection results according to their similarity value
	 * A.
	 */
	private static class SimilarityAComparator implements Comparator, Serializable {
	 ...
	 * B.
	 */
	private static class SimilarityBComparator implements Comparator, Serializable {
		public int compare(Object o1, Object o2) {
			SubmissionDetectionResult r1 = (SubmissionDetectionResult) o1;
			SubmissionDetectionResult r2 = (SubmissionDetectionResult) o2;
	 ...
	 * values.
	 */
	private static class SimilarityProductComparator implements Comparator, Serializable {
		public int compare(Object o1, Object o2) {
			SubmissionDetectionResult r1 = (SubmissionDetectionResult) o1;
			SubmissionDetectionResult r2 = (SubmissionDetectionResult) o2;
	...
	/**
	 * Compares submisison detection results using the maximum file comparison
	 * result within them.
	 */
	private static class MaximumFileSimilarityComparator implements Comparator, Serializable {
		public int compare(Object o1, Object o2) {
			SubmissionDetectionResult r1 = (SubmissionDetectionResult) o1;
			SubmissionDetectionResult r2 = (SubmissionDetectionResult) o2;
	...
	/**
	 * Generates a report of the given submission in sub directory sub[n].
	 * Currently the report only contains the similarity distribution of the
	 * given submission against others.
	 */
	private void generateSubmissionReport(Submission sub, int n) throws Exception {
	...
	/**
	 * Generates a report of submission similarity detection. The configuration
	 * and statistics are added to the report by using the corresponding
	 * HtmlPrintable parameters.
	 */
	public void generateReport(List detResults, List submissions, HtmlPrintable configuration, HtmlPrintable statistics)
			throws Exception {
		// Limit the number of reported detection results, if they exceed the
		// maximum
		...
		if (detResults.size() > this.maxDetResultsToReport) {
			...
			while ((iter.hasNext()) && (c < this.maxDetResultsToReport)) {
				SubmissionDetectionResult subDet = (SubmissionDetectionResult) iter.next();

...
/**
 * This utility class has many methods to manipulate files (unzip, copy, delete,
 * etc.)
 *
 */
public class FilesUtility {
	...
	/**
	 * Extracts specific files (in the fileNames list) from a zip file specified
	 * by the zipFilePath to a directory specified by destDirectory (will be
	 ...
	 * @throws IOException
	 */
	public void unzip(File studentZipFile, File projectFolder,
		...
		while (entry != null) {
			...
			String filePath = projectFolder.getAbsolutePath() + File.separator
					+ entry.getName();
			if (!entry.isDirectory()) {
				// if the entry is a file and is in the list of files to be
				// copied, extracts it
				...
				if (submittedFileName.endsWith(".java")) {
					...
					if (index != -1) {
						String targetFolder = this.filesFolders
								.get(submittedFileName);
						if (targetFolder != null) {
							filePath = projectFolder.getAbsolutePath()
							...
							throw new RuntimeException(
									"File "
											+ submittedFileName
											+ " is not present in the folder structure!");
						}
	...
	/**
	 * Extracts all files from a zip file in a to a directory specified by
	 * destDir (will be created if does not exists). This is intended to create
	 * the initial folder structure of the project/submission to be analysed.
	 * 
	 * @param zipFilePath
	 * @param destDirectory
	 * @throws IOException
